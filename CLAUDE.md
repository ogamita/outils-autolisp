...
- Ce dépôt rassemble plusieurs sous-projets indépendants; il faut les traiter comme des modules séparés, même s'ils partagent le même dépôt Git.
- Chaque sous-projet doit conserver sa propre structure `src/`, `docs/`, `tests/`, etc., et ses propres règles de fonctionnement.
- On peut garder un `Makefile` à la racine de `outils-autolisp` pour quelques cibles générales communes, comme la documentation ou des tests de haut niveau.
- Chaque sous-répertoire de sous-projet doit aussi avoir son propre `Makefile` et ses propres tests indépendants.
