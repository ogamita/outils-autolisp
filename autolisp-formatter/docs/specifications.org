...
* Style supplémentaire de parenthèses : nail-clipping à fermetures empilées
...
* Exceptions de casse par fichier de symboles
Dans certains projets, un petit ensemble de symboles doit conserver une
capitalisation précise pour de bonnes raisons pratiques, par exemple :
...
Le formateur doit donc permettre l'usage d'un *fichier d'exceptions de
capitalisation*.

** Format du fichier d'exceptions
Le fichier contient un symbole par ligne, exactement tel qu'il doit être
réécrit dans la sortie.
...
Les lignes vides sont autorisées et doivent être ignorées.
Une extension future pourra autoriser des commentaires dans ce fichier, mais ce
n'est pas requis pour la première version.

** Sémantique
Lorsque ce fichier est fourni :

- chaque occurrence d'un symbole du source doit être comparée aux entrées du
  fichier de manière insensible à la casse ;
- si un symbole du source correspond à une entrée du fichier, il doit être
  réécrit exactement comme dans le fichier ;
- cette réécriture est prioritaire sur la politique générale de casse des
  symboles.
...
- mode de diagnostic permettant de signaler les symboles présents dans le
  fichier d'exceptions mais jamais rencontrés dans le source.

* Fichier de configuration du formateur
Toutes les options du programme doivent pouvoir être fournies soit directement,
soit au moyen d'un *fichier de configuration* passé comme seul argument au
programme ou à la fonction principale de formatage.
...
- de partager facilement les mêmes réglages entre l'outil en ligne de commande
  et l'API AutoLISP ;
- d'inclure les chemins de fichiers auxiliaires, notamment le fichier
  d'exceptions de capitalisation.

** Formes acceptées
Le fichier de configuration peut contenir soit :

1. une seule s-expression représentant la liste complète des options et
   arguments ;
2. ou bien directement les options et arguments, tels quels, sans liste
   englobante.
...
Dans le second cas, l'implémentation peut lire tout le contenu et l'encapsuler
logiquement dans une liste avant analyse.

** Correspondance programme / API
...
L'objectif est d'avoir une source unique de vérité pour l'interprétation des
options.

** Résolution des conflits
Si plusieurs modes de fourniture des options coexistent, la priorité recommandée
est :
...
Toute implémentation doit documenter clairement cette règle et l'appliquer de
manière déterministe.

** Options minimales à prévoir
...
- option pour activer un contrôle de cohérence des exceptions de
  capitalisation ;
- option permettant de faire traiter un contenu de configuration comme une seule
  s-expression ou comme une suite d'éléments à regrouper implicitement.

** Recommandation de mise en oeuvre
L'analyse des options doit être factorisée dans un module unique, utilisé à la
fois par :
...
Ainsi, toutes les variantes d'entrée produisent la même structure interne
d'options normalisées.
