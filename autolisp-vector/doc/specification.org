...
- taille 1000 : =av-length= 2 ms, =length= 1 ms ;
- taille 5000 : =av-length= 2 ms, =length= 6 ms.

Conclusion : l'objectif =O(1)= pour la longueur est atteint et visible.

*** Lecture d'un élément

=av-aref= respecte toujours l'interface visée mais reste trop coûteux en
pratique face à =nth= dans l'implantation actuelle.

Exemples du run de référence :

- taille 100 : =av-aref= 22 ms, =nth= 1 ms ;
- taille 1000 : =av-aref= 94 ms, =nth= 1 ms ;
- taille 5000 : =av-aref= 184 ms, =nth= 3 ms.

Conclusion : la propriété asymptotique seule ne suffit pas ici ; les constantes
cachées de la représentation actuelle dominent encore le coût total.
...
- taille 100 : =av-set-aref= 16 ms, =list-set-at= 19 ms ;
- taille 500 : =av-set-aref= 36 ms, =list-set-at= 93 ms ;
- taille 5000 : =av-set-aref= 186 ms, =list-set-at= 4459 ms.
...
- taille 1000 : =av-vector-push-pop= 207 ms, =cons-cdr= 0 ms ;
- taille 5000 : =av-vector-push-pop= 178 ms, =cons-cdr= 0 ms.
...
L'optimisation par curseur séquentiel a nettement amélioré =av-subseq=, mais
pas au point de battre l'approche liste.

Exemples du run de référence :

- taille 500 : =av-subseq= 1047 ms, =list-subseq= 32 ms ;
- taille 1000 : =av-subseq= 2366 ms, =list-subseq= 3 ms ;
- taille 5000 : =av-subseq= 2868 ms, =list-subseq= 169 ms.
...
- taille 500 : =av-replace-mid= 4446 ms, =list-replace-mid= 71 ms ;
- taille 1000 : =av-replace-mid= 10008 ms, =list-replace-mid= 146 ms ;
- taille 5000 : =av-replace-mid= 13640 ms, =list-replace-mid= 4 ms.
...
- =av-set-aref= devient intéressant sur grandes tailles ;
- =av-subseq= et =av-replace= ont progressé mais restent trop coûteux ;
- =av-aref= est encore trop lent pour valider l'objectif pratique du projet.

** Interprétation technique

L'explication la plus probable du coût résiduel est la combinaison de :

- stockage des métadonnées dans la valeur d'un symbole ;
- accès indirects via =eval= et =set= dans le chemin critique ;
- structure interne fondée sur symboles + alists ;
- multiplication des helpers et allocations intermédiaires.

Cette baseline doit donc être considérée comme :

- une *baseline fonctionnelle valide* ;
- une *baseline asymptotique partiellement convaincante* ;
- mais *pas encore* une baseline de performance satisfaisante pour la lecture.
